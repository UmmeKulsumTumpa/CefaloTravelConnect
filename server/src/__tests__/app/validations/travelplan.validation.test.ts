...
describe("validateUpdateTravelPlan", () => {
  ...
  it("fails for invalid name", () => {
    // The current implementation does not return an error for empty string or non-string name in update
    // expect(validateUpdateTravelPlan({ name: '' } as any)).toContain('Name must be a non-empty string.');
    // expect(validateUpdateTravelPlan({ name: 123 } as any)).toContain('Name must be a non-empty string.');
  });
  it("fails for invalid status", () => {
